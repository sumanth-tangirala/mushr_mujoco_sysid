# System Overview and Config Variants
...
## Config-by-config behavior: what is learned and what it does

All configs live in `configs/` and are consumed by `scripts/train.py`.
Below we summarize, for each config, which parts are **fixed** and which are **learned**, and how they interact with the plant.
...
Below is a more detailed description of each config.
